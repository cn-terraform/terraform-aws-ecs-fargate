...
| <a name="input_container_definition_overrides"></a> [container\_definition\_overrides](#input\_container\_definition\_overrides) | Container definition overrides which allows for extra keys or overriding existing keys. | `map(any)` | `{}` | no |
...
| <a name="input_deployment_controller"></a> [deployment\_controller](#input\_deployment\_controller) | (Optional) Deployment controller | `list(any)` | <pre>[<br/>  {<br/>    "type": "ECS"<br/>  }<br/>]</pre> | no |
...
| <a name="input_lb_http_ports"></a> [lb\_http\_ports](#input\_lb\_http\_ports) | Map containing objects with two fields, listener\_port and the target\_group\_port to redirect HTTP requests | `map(any)` | <pre>{<br/>  "default-http": {<br/>    "listener_port": 80,<br/>    "target_group_port": 80<br/>  }<br/>}</pre> | no |
...
| <a name="input_lb_https_ports"></a> [lb\_https\_ports](#input\_lb\_https\_ports) | Map containing objects with two fields, listener\_port and the target\_group\_port to redirect HTTPS requests | `map(any)` | <pre>{<br/>  "default-https": {<br/>    "listener_port": 443,<br/>    "target_group_port": 443<br/>  }<br/>}</pre> | no |
